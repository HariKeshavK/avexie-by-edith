<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_avexie_version ?? '0.0.0', WEBUI_VERSION)) {
			...
			toast.error(
				$i18n.t(
					'AVEXIE version (v{{AVEXIE_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						AVEXIE_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_avexie_version ?? '0.0.0'
					}
				)
			);
			return;
