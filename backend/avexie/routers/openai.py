...
from avexie.utils.access_control import check_model_access, has_connection_access, has_permission
from avexie.utils.anthropic import ANTHROPIC_VERSION, get_anthropic_models, is_anthropic_url
...
async def get_headers_and_cookies(
    ...
    headers = {
        ...
        **(
            {
                # LICENSE covers this AVEXIE upstream metadata identifier.
...
async def speech(request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        try:
            ...
            # LICENSE covers this AVEXIE error identifier.
            # Do not alter, remove, obscure, or replace it except as LICENSE permits:
            # https://docs.openwebui.com/license.
            raise HTTPException(
                status_code=r.status if r else 500,
                detail=detail if detail else 'AVEXIE: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                # Do not alter, remove, obscure, or replace it except as LICENSE permits:
                # https://docs.openwebui.com/license.
                raise HTTPException(status_code=500, detail='AVEXIE: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    AVEXIE stores extra bookkeeping fields (``id``, ``status``,
        ...
        if isinstance(content, str):
            ...
            for part in content:
                if part.get('type') == 'text':
                    ...
                    # OpenAI-compatible proxy path only. AVEXIE attachments are handled
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='AVEXIE: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
